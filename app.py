...
def translate_text(text, api_key, source_language="tr", target_language="ar"):
    """Metni hedef dile çevirir - Sadece OpenAI"""
    try:
        language_names = {
            "tr": "Türkçe",
            "en": "İngilizce", 
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system", 
                    "content": f"Sen bir profesyonel çevirmensin. Verilen {source_name} metni {target_name}'ya çevir. Sadece çeviriyi döndür, başka açıklama yapma."
                },
                {
                    "role": "user", 
                    "content": f"Bu metni {source_name}'dan {target_name}'ya çevir: {text}"
                }
            ],
            temperature=0.3
        )
        ...
        st.error(f"OpenAI çeviri hatası: {str(e)}")
        return None
...
def main():
    st.set_page_config(
        page_title="Çok Dilli Ses Çevirici - OpenAI",
        page_icon="🌍",
        layout="wide"
    )
    
    st.title("🌍 Çok Dilli Ses Çevirici")
    st.markdown("**OpenAI API** kullanarak ses dosyalarını ve metinleri Türkçe, İngilizce ve Arapça arasında çevirin")
    ...
    with st.sidebar:
        ...
        source_lang_name = st.selectbox(
            ...
            help="Çevrilecek metnin/sesin dili"
        )
        
        target_lang_name = st.selectbox(
            "Hedef dil:",
            ["Arapça", "İngilizce", "Türkçe"],
            help="Çevrilmek istenen dil"
        )
        ...
        if source_lang == target_lang:
            st.warning("⚠️ Kaynak ve hedef dil aynı olamaz!")
